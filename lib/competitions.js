...
async function get_games_from_matchday(liga, season, matchday) {
    const result = await query2(`SELECT * FROM ${liga} WHERE Saison = '${season}' AND Spieltag = ${matchday} ORDER BY ID`);
    return result;
}
...
async function update_ui() {
    ...
    var games = await get_games_from_matchday(comp, "2023-2024", matchday);
    ...
    for (var i = 0; i < games.length; i++) {
        ...
        add_game(league, th, result, ta, date, time);
    }
}
...
function add_game(league, th, res, ta, date, time) {
    const all_games = document.getElementById("all-games");
    ...
    row = createGameRow(league, th, res, ta);
...
function createGameRow(league, th, res, ta) {
    trikot1 = create_img('assets/trikots/' + league + '/' + th + '.png');
    th_div = create_div(th, "th");
    res_div = create_div(res, "result");
    ta_div = create_div(ta, "ta");
    trikot2 = create_img('assets/trikots/' + league + '/' + ta + '.png');
